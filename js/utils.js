/**
 * Create a class or an object with an inheritance from a class or an object
...
;+function(window, undefined){
	...
	window.Pish = (function(pish) {
			pish.prototype.pish = function(Pish, prot, face, sngl) {
				var line = null,
				file = null,
				column = null,
				stack = null;
			...
			if(Pish instanceof Array) {
				var args = [],
					win = (function() {return this;})();
				Pish.forEach(function(val, arg) {
					switch(val) {
						case 'undefined':
							arg = void(0);
						break;
						...
						break;
						default:
							arg = eval('(function(){try{return '+val+' != void(0) ? '+val+' : void(0);}catch(e){return void(0);}})()');
							if(arg == void(0)) {
								throw new Error('"'+val+'" required'+(file!=null?' in '+file+' on line '+line+', column '+column:'')+'!');
							}
						break;
					}
					args.push(arg);
				});
				Pish = prot.apply(this, args);
				prot = Pish[1];
				face = Pish[2];
				sngl = Pish[3];
				Pish = Pish[0];
...
;+function(window, undefined){
	...
	window.utils = {
		 ...
		 * @return {HTMLElement}
		 */
		toDOM: function(s,e,f) {
			e = this.node('p', {html: s});
			f = this.node('fragment');
			while(s = e.firstChild) f.appendChild(s);
			return f;
		},

		/**
		 * Compares objects and return properties that are differ
		 ...
		 */
		compare: function(newObj, oldObj) {
				...
				check = function(o1, o2) {
					for(prop in o1) {
						...
						if(o1[prop] instanceof Date){
							...
							if(result = this.compare(newObj[prop], oldObj[prop])){
		...
		var options = {
			...
			publ = function tip(elem){
				if(!this || this===window) { // parse
					...
					this.background = utils.node('div');
					...
					utils.classes.add(this.background, 'tip-background');
